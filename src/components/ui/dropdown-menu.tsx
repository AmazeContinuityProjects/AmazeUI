...
function DropdownMenu({ children, open: controlledOpen, onOpenChange }: DropdownMenuProps) {
  ...
  return (
    <DropdownContext.Provider value={{ open, setOpen }}>
      <View {...({ className: "relative" } as any)}>{children}</View>
...
const DropdownMenuContent = React.forwardRef<
  ...
  ViewProps & { className?: string; align?: "start" | "end" | "center"; sideOffset?: number }
...
const DropdownMenuItem = React.forwardRef<
  React.ElementRef<typeof Pressable>,
  React.ComponentProps<typeof Pressable> & { className?: string; textClassName?: string }
>(({ className, textClassName, onPress, children, ...props }, ref) => {
  ...
  return (
    <Pressable
      ...
      {...({
        className: cn(
          "relative flex-row items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-accent focus:bg-accent",
          className
        ),
      } as any)}
      {...props}
...
>(({ className, ...props }, ref) => (
  <Text
    ref={ref}
    {...({ className: cn("px-2 py-1.5 text-sm font-semibold text-foreground", className) } as any)}
    {...props}
  />
))
